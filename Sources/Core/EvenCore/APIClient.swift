...
public final class EvenAPIClient: @unchecked Sendable {
    ...
    public func get<T: Decodable>(_ path: String) async throws -> T {
        try await request("GET", path, body: Int?.none)
    }
    ...
    public func post<T: Decodable>(_ path: String) async throws -> T {
        try await request("POST", path, body: Int?.none)
    }
...
public extension EvenAPIClient {
    ...
    func summary() async throws -> Summary {
        try await get("v1/summary")
    }

    struct TaskDraftBody: Encodable, Sendable {
    ...
    func pendingDrafts() async throws -> [Draft] {
        try await get("v1/drafts?status=pending")
    }
    ...
    struct DraftPatchBody: Encodable, Sendable {
        public var title: String?
        public var ownerMemberId: UUID?
        public var amountCents: Int?
        public var dueOn: String?
    ...
    struct ApproveResponse: Decodable, Sendable {
        ...
        public init(draft: Draft, task: HouseholdTask) {
            self.draft = draft
            self.task = task
        }
    }
    ...
    func money() async throws -> Money {
        try await get("v1/money")
    }
    ...
    func settle() async throws -> Money {
        try await post("v1/settle")
    }

    func resetSummary() async throws -> ResetSummary {
        try await get("v1/reset")
    }
    ...
    func googleStatus() async throws -> GoogleStatus {
        try await get("v1/google/status")
    }

    func googleDisconnect() async throws {
        struct Empty: Decodable {}
        let _: Empty = try await post("v1/google/disconnect")
    }

    /// Starts the async scan job; 409 sync_running means one is already live.
    func googleSync() async throws -> GoogleSyncStart {
        try await post("v1/google/sync")
    }
