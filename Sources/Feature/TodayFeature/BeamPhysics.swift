...
final class TiltGravityProvider {
    ...
    #else
        func start(onTilt _: @escaping (CGFloat) -> Void) {}
        func stop() {}
    #endif
}
...
final class BeamPhysicsScene: SKScene {
    ...
    private func restyle() {
        ...
        for ball in meBalls {
            ball.fillColor = meColor; ball.strokeColor = meColor
        }
        for ball in partnerBalls {
            ball.fillColor = partnerColor; ball.strokeColor = partnerColor
        }
    }
...
public struct BeamScaleView: View {
    public let summary: Summary
    public let me: Member?
    public let partner: Member?
    ...
    public init(summary: Summary, me: Member?, partner: Member?) {
        self.summary = summary
        self.me = me
        self.partner = partner
    }

    private var meColor: Color {
        color(for: me?.color) ?? EvenTokens.terracotta
    }

    private var partnerColor: Color {
        color(for: partner?.color) ?? EvenTokens.pine
    }
    ...
    public var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                Capsule().fill(EvenTokens.espresso).frame(width: 120, height: 2).position(x: cx, y: 196)
                ...
                Color.clear.frame(width: 1, height: 1)
                    ...
                    .accessibilityLabel((me?.displayName ?? "You").uppercased())
                    .accessibilityAddTraits(.isStaticText)
                Color.clear.frame(width: 1, height: 1)
                    .position(x: cx + 100, y: 40)
                    .accessibilityLabel((partner?.displayName ?? "— ?").uppercased())
                    .accessibilityAddTraits(.isStaticText)
                ...
                RollingNumber(value: summary.percentPartner)
                    .font(.system(size: 34, weight: .medium, design: .serif))
                    .monospacedDigit()
                    .foregroundStyle(partner == nil ? AnyShapeStyle(EvenTokens.stone.opacity(0.6))
                        : AnyShapeStyle(partnerColor))
                    .position(x: cx + 128 * unit(geo), y: 34)
            }
            .onChange(of: summary.percentMe) { configureScene() }
            .onChange(of: summary.pebbles) { configureScene() }
        }
    }
    ...
    private func color(for memberColor: MemberColor?) -> Color? {
        switch memberColor {
        case .clay: EvenTokens.terracotta
        case .teal: EvenTokens.pine
        case .none: nil
        }
    }
    ...
    private func configureScene() {
        scene.apply(
            ...
            ghostPartner: partner == nil,
            meName: me?.displayName ?? "You",
            partnerName: partner?.displayName ?? "\u{2014} ?"
        )
        scene.syncBalls(me: weights(for: me?.id), partner: weights(for: partner?.id))
    }
